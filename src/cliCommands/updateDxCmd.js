...
function registerCommand ({ cli }) {
  cli.command('update-dx [--conf file]', 'Modifies DX proxy of the DX module. For this operation, if MNEMONIC or private keys provided, it should have ownership of the safe.', yargs => {
    ...
    await validateUpdateDx(jsonConf)
    ...
    const multisigData = moduleInstance.changeDXProxy.request(dxProxy.address).params[0].data
    ...
    const contractDxProxy = await moduleInstance.dutchXAddress()
    if (contractDxProxy == dxProxy.address) {
    ...
    if (process.env.MNEMONIC || process.env.PK){
        ...
        const { confirmation } = await inquirer.prompt(
        {
            name: "confirmation",
            type: "confirm",
            message: `Update dx proxy address based on these parameters?`
        }
        )
        if(!confirmation){
            logger.info("Exit.")
